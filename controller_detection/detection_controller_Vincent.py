...
class DetectionController:
    def __init__(self):
        ...
        # Detection
        self.forget_time: float = 3.0 # seconds
        ...
        # Tracking
        self.redetect_timeout: float = 0.0 # seconds
        self.track_error_threshold = 50.0 # pixels
        ...
        self.active_align_state = "ALTITUDE" # ALTITUDE, FORWARD, ORBIT
    ...
    def closed_loop_control(self, detections: list[list[CornerDetection]]) -> tuple[list, bool]:
        """
        Executes the active vision search pattern.
        Loops continuously: ALTITUDE -> FORWARD <-> ORBIT
        until the background triangulation confirms the gate.
        """
        x, y, z = self.current_drone_xyz
        _, _, yaw = self.current_drone_rpy
        ...
        # 1. ZONE CHECK
        zone_id = self._get_zone(x, y) 
        
        # 2. DEBUG VISUALIZATION 
        if self.debug_mode:
            print(f"Current Zone: {zone_id} | Gates detected: {len(detections) if isinstance(detections, list) else 0}")
            
            for det in detections if isinstance(detections, list) else []:
                for cd in det:
                    if cd.corner_id == CornerID.BL:
                        print(f"  -> Detected BL corner at pixel ({cd.uv[0]:.1f}, {cd.uv[1]:.1f}) with conf {cd.conf:.2f}")

        # 3. ACTIVE VISION ALIGNMENT
        tracking_candidates = [c for c in self.gate_candidates.values() if c.state == CandidateState.TRACKING]
        
        if tracking_candidates:
            best_candidate = tracking_candidates[0] 
            pts = best_candidate.last_tracked_pts
            
            if pts and len(pts) == 4:
                tl, bl = pts[CornerID.TL], pts[CornerID.BL]
                tr, br = pts[CornerID.TR], pts[CornerID.BR]
                ...
                # YAW ALIGNMENT (Always Active)
                center_x = self.K[0, 2] 
                yaw_error = center_x - avg_u
                new_yaw = yaw + yaw_error * 0.015 

                # --- THE CONTINUOUS APPROACH STATE MACHINE ---
                ...
                # Safety override: If altitude gets knocked severely out of bounds, fix it immediately
                if abs(vertical_error) > 25.0:
                    self.active_align_state = "ALTITUDE"

                # State 1: Altitude Hold
                if self.active_align_state == "ALTITUDE":
                    if self.debug_mode: print("  [Align] Adjusting Altitude...")
                    new_z = z + (vertical_error * 0.002) 
                    
                    # Transition: If altitude is good, begin the forward approach
                    if abs(vertical_error) <= 15.0: 
                        self.active_align_state = "FORWARD"

                # State 2: Forward Cruise
                elif self.active_align_state == "FORWARD":
                    if self.debug_mode: print("  [Align] Cruising Forward...")
                    
                    # Continuously push the setpoint 15cm ahead of current position
                    new_x = x + 0.15 * np.cos(yaw)
                    new_y = y + 0.15 * np.sin(yaw)
                    
                    # Transition: Did we drift horizontally off-center while flying?
                    if abs(height_diff) > alignment_tolerance:
                        self.active_align_state = "ORBIT"

                # State 3: Horizontal Orbit Fix
                elif self.active_align_state == "ORBIT":
                    if abs(height_diff) > alignment_tolerance:
                        strafe_speed = 9.0 / (max_h + 1e-5) 
                        
                        if height_diff > 0:
                            if self.debug_mode: print("  [Align] Orbiting Right to fix alignment...")
                            new_x = x + strafe_speed * np.sin(yaw)
                            new_y = y - strafe_speed * np.cos(yaw)
                        else:
                            if self.debug_mode: print("  [Align] Orbiting Left to fix alignment...")
                            new_x = x - strafe_speed * np.sin(yaw)
                            new_y = y + strafe_speed * np.cos(yaw)
                    else:
                        # Transition: We are centered again! Resume cruising forward.
                        if self.debug_mode: print("  [Align] Orbit fixed, resuming Forward approach.")
                        self.active_align_state = "FORWARD"
                        
            else:
                if self.debug_mode: print("  [Failsafe] Gate partially obscured, holding position.")
                pass
                
        else:
            # ─── BLIND SEARCH (Spin and drift) ───────────────────────────────
            self.active_align_state = "ALTITUDE" # Reset loop for the next gate
            
            new_yaw = yaw + 0.2
            
            center_x = self.room_x / 2.0
            center_y = self.room_y / 2.0
            vector_x = center_x - x
            vector_y = center_y - y
            ...
            if dist_to_center > 0.5:
                drift_speed = 0.15 
                new_x = x + (vector_x / dist_to_center) * drift_speed
                new_y = y + (vector_y / dist_to_center) * drift_speed
                
            if z < 0.65 or z > 0.75:
                new_z = z + (0.7 - z) * 0.8

        # 4. EXIT CONDITION
        # The moment the background math verifies the gate in 3D, this triggers true
        is_gate = len(self.detected_gates) > 0

        return [new_x, new_y, new_z, new_yaw], is_gate
    ...
    def debug_visualization(self) -> None:
        ...
        # Draw gate candidates with different colors based on state
        for candidate in self.gate_candidates.values():
            color = (0, 255, 0) if candidate.state == CandidateState.CONFIRMED else (0, 255, 255) if candidate.state == CandidateState.TRIANGULATING else (255, 0, 0)
            for obs_list in candidate.observations.values():
                for obs in obs_list:
                    cv2.circle(vis, tuple(obs.uv.astype(int)), 5, color, -1)

        # Draw detected gates
        for gate in self.detected_gates.values():
            for obs_list in gate.observations.values():
                for obs in obs_list:
                    cv2.circle(vis, tuple(obs.uv.astype(int)), 5, (0, 255, 0), -1)
    ...
    def compute_command(self, sensor_data, camera_data, dt):
        ...
        ### SEARCH ###
            ...
            if not self.set_path:
                ...
                #search_spin_rate = 0.2
                #self.current_setpoint = ([sensor_data['x_global'], sensor_data['y_global'], 1, sensor_data['yaw'] + search_spin_rate])
